...
Adding the `background-check` step to your flow will run a background check on U.S.-based users over 18, including criminal history and sex offender registry. The status is available via `background_check.status` with values: `not_started`, `started`, `pending`, `consider`, `pass`, `failed`, `canceled`, `expired`, or `information_mismatch`. If the check fails, `user.has_criminal_activity` will be `true` and an email notification is sent to the app.
