...
## 2. Create a Payment Customer <span style={{fontSize: 16, paddingLeft: 10}}>`server-side`</span> <span style={{fontSize: 16, paddingLeft: 10}}>`optional`</span>

<Note>
  You may skip this step if you do not intend to integrate payment method reusability for your customers.
</Note>

When a payment is created, a payment method is generated and saved to the [PaymentIntent](/payment-intents/create-a-payment-intent) object. This payment method is only capable of being used once (i.e. single-use) unless it is attached to a [PaymentCustomer](/payment-customers/create-a-payment-customer), in which case it becomes reusable.
...
## 3. Create a Payment Intent <span style={{fontSize: 16, paddingLeft: 10}}>`server-side`</span>
...
-d "{
      "amount": 1000,
      "currency": "usd",
      "customer_id": "0e835300-5555-4ba1-816a-0abaa8609a83", // or use "user_id"
...
## 4. Collect card details <span style={{fontSize: 16, paddingLeft: 10}}>`client-side`</span>
...
## 5. Submit payments to Dots <span style={{fontSize: 16, paddingLeft: 10}}>`client-side`</span>

Rather than sending the entire PaymentIntent object to the client, use its client secret from [step 3](#3-create-a-payment-intent). After you create the PaymentIntent, the API returns a unique client secret for it (the client secret is different from the secret key you use to make Dots API requests).
...
## 6. Create future payments with reusable payment method. <span style={{fontSize: 16, paddingLeft: 10}}>`optional`</span>
...
When a customer completed the first payment, a payment method will be saved to that payment intent. You can listen to the webhook event `payment_intent.updated`, and [retrieve the payment intent](/payment-intents/retrieve-a-payment-intent) object to find the `payment_method_id`. Save that id to your user on your database. 
