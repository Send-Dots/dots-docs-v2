...
    3. Choose the **Organization**, provide a **App name**, and click **Create**.
...
  </Tab>
  <Tab title="API">
 To create the Payout Link using the API, you will need to use the **App API Key** and **Client ID**. You can find this information by accessing the Dashboard, selecting the desired App, and then selecting the **API Management** on the sidebar. 
    
...
| `delivery.method` | Delivery method. Options: `sms`, `link`, `email`, `all`. `sms` |
...
    </CodeGroup>
 ...
 The `link` parameter in the response will provide the Paylout Link you can share with the payee.
