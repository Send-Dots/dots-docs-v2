...
When the user completes each step, a webhook with the flow's `id` will be triggered. You can then use the the [`/v2/flows/{flow_id}`](/api-reference/v2/flows/get-flow-by-id) endpoint to retrieve the user's `id`.
