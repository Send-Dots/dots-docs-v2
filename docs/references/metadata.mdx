...
You can include a metadata object in your API request with up to 50 key-value pairs. Each key can be up to 40 characters long, and each value can be up to 500 characters long. This metadata is returned in the response to your request. The following example demonstrates a request to create a user where the metadata is used to store the user's internal_id.
...
The following code block presents a `metadata` example for a iphotetical payment for a vendor related to a sale.
