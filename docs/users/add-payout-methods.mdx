...
Payout methods can be added by a user's account by the user through the UI components provided by Dots. For a more custom integration, you can use the [/v2/users/\{user_id\}/payout-methods](/apireference/users/add-payout-method) endpoint to do the same.
