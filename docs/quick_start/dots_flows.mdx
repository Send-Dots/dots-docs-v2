...
<Note>The `onboarding` step is automatically added to the start of all flows if a `user_id` is not provided.
...
## Step 3: Create a flow for the user to withdraw funds

Now that we have the user's `id`, we can create a flow for the user to withdraw funds.
...
The link will stay autneticated for 15 minutes. If the user takes longer than 15 minutes to complete the flow, they will need to authenticte through an sms token.
